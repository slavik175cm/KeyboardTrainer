...
User* DataBase::get_user(QString name) {
    query->exec("SELECT * FROM letters_accuracy WHERE name='" + name + "';");
    query->next();
    ...
    for (int i = 0; i < 26; i++) {
        user->letter_pressed_count[i] = query->value(rec.indexOf(QString('A' + i) + "_pressed")).toInt();
...
void DataBase::add_user(User *user) {
    ...
    qry = "INSERT INTO letters_accuracy (name, password";
    for (int i = 0; i < 26; i++) {
    ...
    for (int i = 0; i < 26; i++) {
        qry += ", " + QString::number(user->letter_pressed_count[i]);
...
void DataBase::update(User *user) {
    qry = "UPDATE letters_accuracy SET ";
    for (int i = 0; i < 26; i++) {
        if (i != 0) qry += ", ";
        qry += QString('A' + i) + "_pressed = '" + QString::number(user->letter_pressed_count[i]) + '\'';
...
bool DataBase::is_name_exist(QString name) {
    query->exec("SELECT COUNT(*) FROM letters_accuracy WHERE name='" + name +"';");
    query->next();
...
bool DataBase::is_password_correct(QString name, QString password) {
    ...
    query->exec("SELECT password FROM letters_accuracy WHERE name='" + name +"';");
    if (!query->next())
        return false;
